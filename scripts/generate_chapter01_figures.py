...
def proof_boundaries() -> None:
    ...
    draw.text((60, 45), "三条证据链：谁能证明什么", font=font(42), fill="#0f1e3a")
    draw.text(
        (62, 100),
    ...
    boxes = [(70, 190, 330, 340), (470, 190, 730, 340), (870, 190, 1130, 340)]
    ...
    for box, (fill, outline), title, subtitle in zip(boxes, colors, titles, subtitles):
        ...
        center(draw, (box[0], box[1] + 5, box[2], box[3] - 35), title, font(30))
        center(draw, (box[0], box[3] - 45, box[2], box[3] - 10), subtitle, font(18), fill=(74, 85, 104))

    arrow(draw, (335, 265), (465, 265))
    arrow(draw, (735, 265), (865, 265))

    rounded(draw, (210, 500, 990, 650), "#eef2ff", "#7c3aed")
    center(draw, (210, 510, 990, 590), "人的发布决定", font(32), fill=(49, 24, 87))
    center(
        draw,
        (210, 585, 990, 640),
    ...
    arrow(draw, (600, 345), (600, 495), color=(124, 58, 237))
    draw.text(
        (120, 700),
    ...
    image.save(OUT / "chapter-01-proof-boundaries.png")


def confidence_trap() -> None:
    image = Image.new("RGB", (1200, 760), "#fffdf7")
    draw = ImageDraw.Draw(image)
    draw.text((60, 45), "confidence 不是未来盈利概率", font=font(42), fill="#111827")
    draw.text(
        (62, 100),
    ...
    for index, (x1, y1, x2, y2, text, fill, outline) in enumerate(steps):
        ...
        center(draw, (x1, y1, x2, y2), text, font(24))
        if index < len(steps) - 1:
            arrow(draw, (x2 + 10, 290), (steps[index + 1][0] - 10, 290), color=(220, 38, 38))

    rounded(draw, (110, 500, 1090, 640), "#ecfdf5", "#059669")
    center(draw, (110, 510, 1090, 575), "正确处理：只把 confidence 当成表达强度", font(30), fill=(6, 95, 70))
    center(
        draw,
        (110, 580, 1090, 632),
    ...
    draw.text((120, 700), "读图：高置信度先触发复核，而不是触发下单。", font=font(22), fill="#374151")
    image.save(OUT / "chapter-01-confidence-trap.png")
